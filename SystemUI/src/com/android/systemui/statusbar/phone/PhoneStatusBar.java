...
public class PhoneStatusBar extends BaseStatusBar {
	...
	private final GestureRecorder mGestureRec = DEBUG_GESTURES ? new GestureRecorder(
			"/sdcard/statusbar_gestures.dat") : null;
	...
	private ContentObserver mUserSetupObserver = new ContentObserver(
			new Handler()) {
		@Override
		public void onChange(boolean selfChange) {
			final boolean userSetup = 0 != Settings.Secure.getIntForUser(
					mContext.getContentResolver(),
					Settings.Secure.USER_SETUP_COMPLETE, 0 /* default */,
					mCurrentUserId);
			if (MULTIUSER_DEBUG)
	...
	protected PhoneStatusBarView makeStatusBarView() {
		...
		PanelHolder holder = (PanelHolder) mStatusBarWindow
				.findViewById(R.id.panel_holder);
		...
		mSystemIconArea = (LinearLayout) mStatusBarView
				.findViewById(R.id.system_icon_area);
		...
		mStatusBarContents = (LinearLayout) mStatusBarView
				.findViewById(R.id.status_bar_contents);
		mTickerView = mStatusBarView.findViewById(R.id.ticker);
		mCenterClock = mStatusBarView.findViewById(R.id.clock_center);
		mPile = (NotificationRowLayout) mStatusBarWindow
		...
		mSettingsButton = (ImageView) mStatusBarWindow
		...
		if (mSettingsButton != null) {
			...
			if (mHasSettingsPanel) {
				if (mStatusBarView.hasFullWidthNotifications()) {
					...
					// the (large) screen
					final View buttonHolder = mStatusBarWindow
		...
		// setup the battery view
		mContext.getContentResolver().registerContentObserver(
				Settings.System.getUriFor(Settings.System.BATTERY_TEXT_SIZE),
				false, mBatteryTextObserver);
		setupBatteryView();

		mEmergencyCallLabel = (TextView) mStatusBarWindow
		...
		if (mHasSettingsPanel) {
			...
			if (mHasFlipSettings) {
				...
				// notifications
				settings_stub = mStatusBarWindow
						.findViewById(R.id.flip_settings_stub);
				...
				settings_stub = mStatusBarWindow
						.findViewById(R.id.quick_settings_stub);
				if (settings_stub != null) {
					mSettingsPanel = (SettingsPanelView) ((ViewStub) settings_stub)
							.inflate();
				} else {
				...
				if (mSettingsPanel != null) {
					if (!ActivityManager.isHighEndGfx()) {
						mSettingsPanel
								.setBackground(new FastColorDrawable(
			...
			mSettingsContainer = (QuickSettingsContainerView) mStatusBarWindow
		...
		mClingShown = !(DEBUG_CLINGS || !Prefs.read(mContext).getBoolean(
				Prefs.SHOWN_QUICK_SETTINGS_HELP, false));
	...
	private void setupBatteryView() {
		LinearLayout signalCluster = (LinearLayout) mSystemIconArea
				.findViewById(R.id.signal_battery_cluster);
		if (mBattery != null) {
			signalCluster.removeView(mBattery);
		}

		mBattery = View.inflate(mContext, R.layout.battery_with_text, null);
		signalCluster.addView(mBattery, 1);

		mBatteryController.clearViews();
		mBatteryController.addIconView((ImageView) mBattery
				.findViewById(R.id.battery));
		mBatteryController.addLabelView((TextView) mBattery
				.findViewById(R.id.battery_text));
	}

	private ContentObserver mBatteryTextObserver = new ContentObserver(mHandler) {
		@Override
		public void onChange(boolean selfChange) {
			setupBatteryView();
		}
	...
	private ContentObserver mDirtySettingsObserver = new ContentObserver(
			mHandler) {
		@Override
		public void onChange(boolean selfChange) {
	...
	class SettingsObserver extends ContentObserver {
		...
		void observe() {
			...
			resolver.registerContentObserver(Settings.System
					.getUriFor(Settings.System.SHOW_NAVBAR_SEARCH), false, this);
			resolver.registerContentObserver(Settings.System
			...
			resolver.registerContentObserver(Settings.System
					...
					this);
			resolver.registerContentObserver(Settings.System
					.getUriFor(Settings.System.SHOW_STATUSBAR_CLOCK), false,
					this);
			resolver.registerContentObserver(Settings.System
			...
			resolver.registerContentObserver(
					...
					false, this);
			resolver.registerContentObserver(
					Settings.System.getUriFor(Settings.System.LONG_ACTION_HOME),
					false, this);
			resolver.registerContentObserver(
					Settings.System.getUriFor(Settings.System.NAVBAR_FLIP_OVER),
					false, this);
			resolver.registerContentObserver(Settings.System
	...
	private void buildNavBarView() {
		try {
			...
			if (showNav) {
				mNavigationBarView = (CustomNavigationBarView) View.inflate(
						mContext, R.layout.custom_navigation_bar, null);
	...
	View.OnLongClickListener mHomeLongClickListener = new View.OnLongClickListener() {
		@Override
	...
	View.OnClickListener mHomeClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			mContext.sendBroadcastAsUser(new Intent(
					"com.android.systemui.TOGGLE_QUICKLAUNCH"), UserHandle.ALL);
		}
	};
	...
	private void prepareNavigationBarView() {
		...
		mNavigationBarView.setOnTouchListener(mRecentsPreloadOnTouchListener);
		mNavigationBarView
				.setRecentButtonOnClickListener(mRecentsClickListener);
		if (CustomKeyButtonView.ACTION_DEFAULT.equals(Settings.System
				.getString(mContext.getContentResolver(),
						Settings.System.LONG_ACTION_HOME,
						CustomKeyButtonView.ACTION_DEFAULT)))
	...
	private void updateSearchLightListeners() {
		boolean alwaysOn = Settings.System.getInt(
				mContext.getContentResolver(),
				Settings.System.ALWAYS_QUICK_LAUNCH, 1) == 1;

		mNavigationBarView.getSearchLight().setOnTouchListener(
				alwaysOn ? mHomeSearchActionListener : null);
		mNavigationBarView.getSearchLight().setOnClickListener(
				alwaysOn ? null : mHomeClickListener);
		mNavigationBarView.getSearchLight().setOnLongClickListener(
				alwaysOn ? null : mHomeLongClickListener);
	...
	private void redrawNavigationBar() {
		...
		Settings.System.putInt(mContext.getContentResolver(),
				Settings.System.NAVBAR_ORDER_CHANGED, 0);
		((WindowManagerImpl) mContext.getSystemService(Context.WINDOW_SERVICE))
				.removeView(mNavigationBarView);
		buildNavBarView();
		addNavigationBar();
	}
	...
	private void setNavbarReflections() {
		if (mNavigationBarView != null) {
			mNavigationBarView.setButtonImages(Settings.System.getInt(
					mContext.getContentResolver(),
	...
	private boolean shouldRedraw() {
		...
		if (mNavigationBarView != null) {
			...
			showSearch = Settings.System.getInt(mContext.getContentResolver(),
					Settings.System.SHOW_NAVBAR_SEARCH, 0) == 1;
			hasReflect = mNavigationBarView.mHasReflections;
			showReflect = Settings.System.getInt(mContext.getContentResolver(),
					Settings.System.SHOW_NAVBAR_REFLECTION, 0) == 1;
		}
	...
	public void addNotification(IBinder key, StatusBarNotification notification) {
		 ...
		 * ticker mode // 1. Populate mIntruderAlertView
		 * 
		 * if (notification.notification.intruderView == null) { Slog.e(TAG,
		 * notification.notification.toString() +
		 * " wanted to intrude but intruderView was null"); return; }
		 * 
		 * // bind the click event to the content area PendingIntent
		 * contentIntent = notification.notification.contentIntent; final
		 * View.OnClickListener listener = (contentIntent != null) ? new
	...
	protected void updateCarrierLabelVisibility(boolean force) {
		...
		if (force || mCarrierLabelVisible != makeVisible) {
			...
			mCarrierLabel.animate()
					...
					.setDuration(150)
					.setListener(
							makeVisible ? null : new AnimatorListenerAdapter() {
								@Override
								public void onAnimationEnd(Animator animation) {
	...
	protected void setAreThereNotifications() {
		...
		if (mHasFlipSettings && mFlipSettingsView != null
			...
			mClearButton.setVisibility(clearable ? View.VISIBLE
					: View.INVISIBLE);
		}
	...
	public void updateClock() {
		...
		boolean visible = Settings.System.getInt(mContext.getContentResolver(),
				Settings.System.SHOW_STATUSBAR_CLOCK, 1) == 1;
		// make sure we always disable if the user/app/system requests it
		...
		if ((clock != null) && (clock2 != null)) {
			...
			if (visible && !disabled) {
				if (centerClock) {
					...
					mNotificationIcons.setCenteredClock(true);
				} else {
					clock.setVisibility(View.VISIBLE);
					clock2.setVisibility(View.GONE);
					mNotificationIcons.setCenteredClock(false);
				}
			} else {
				clock.setVisibility(View.GONE);
				clock2.setVisibility(View.GONE);
	...
	public void disable(int state) {
		...
		flagdbg.append(((diff & StatusBarManager.DISABLE_CLOCK) != 0) ? "* "
				: " ");
		flagdbg.append(((state & StatusBarManager.DISABLE_SEARCH) != 0) ? "SEARCH"
				: "search");
		...
		if ((diff & StatusBarManager.DISABLE_SYSTEM_INFO) != 0) {
			...
			if ((state & StatusBarManager.DISABLE_SYSTEM_INFO) != 0) {
				mSystemIconArea.animate().alpha(0f)
				...
				mSystemIconArea.animate().alpha(1f).translationY(0)
						.setStartDelay(0)
						.setInterpolator(new DecelerateInterpolator(1.5f))
		...
		if ((diff & (StatusBarManager.DISABLE_HOME
				...
				| StatusBarManager.DISABLE_BACK | StatusBarManager.DISABLE_SEARCH)) != 0) {
		...
		if ((diff & StatusBarManager.DISABLE_NOTIFICATION_ICONS) != 0) {
			if ((state & StatusBarManager.DISABLE_NOTIFICATION_ICONS) != 0) {
				...
				mNotificationIcons.animate().alpha(0f)
						.translationY(mNaturalBarHeight * 0.5f)
						.setDuration(175)
						.setInterpolator(new DecelerateInterpolator(1.5f))
						.setListener(mMakeIconsInvisible).start();
				...
				mNotificationIcons.animate().alpha(1f).translationY(0)
						.setStartDelay(0)
						.setInterpolator(new DecelerateInterpolator(1.5f))
						.setDuration(175).start();
			}
	...
	public void animateCollapsePanels(int flags) {
		if (SPEW) {
			Slog.d(TAG, "animateCollapse():" + " mExpandedVisible="
	...
	public Animator setVisibilityWhenDone(final Animator a, final View v,
			final int vis) {
	...
	public void flipToNotifications() {
		...
		mScrollViewAnim = start(startDelay(
				FLIP_DURATION_OUT,
				interpolator(
						mDecelerateInterpolator,
						ObjectAnimator.ofFloat(mScrollView, View.SCALE_X, 0f,
								1f).setDuration(FLIP_DURATION_IN))));
		mFlipSettingsViewAnim = start(setVisibilityWhenDone(
				interpolator(
						mAccelerateInterpolator,
						ObjectAnimator.ofFloat(mFlipSettingsView, View.SCALE_X,
								1f, 0f)).setDuration(FLIP_DURATION_OUT),
				mFlipSettingsView, View.INVISIBLE));
		mNotificationButtonAnim = start(setVisibilityWhenDone(
				ObjectAnimator.ofFloat(mNotificationButton, View.ALPHA, 0f)
						.setDuration(FLIP_DURATION), mNotificationButton,
				View.INVISIBLE));
		mSettingsButton.setVisibility(View.VISIBLE);
		mSettingsButtonAnim = start(ObjectAnimator.ofFloat(mSettingsButton,
				View.ALPHA, 1f).setDuration(FLIP_DURATION));
		mClearButton.setVisibility(View.VISIBLE);
		mClearButton.setAlpha(0f);
	...
	public void flipToSettings() {
		...
		mFlipSettingsViewAnim = start(startDelay(
				FLIP_DURATION_OUT,
				interpolator(
						mDecelerateInterpolator,
						ObjectAnimator.ofFloat(mFlipSettingsView, View.SCALE_X,
								0f, 1f).setDuration(FLIP_DURATION_IN))));
		mScrollViewAnim = start(setVisibilityWhenDone(
				interpolator(
						mAccelerateInterpolator,
						ObjectAnimator.ofFloat(mScrollView, View.SCALE_X, 1f,
								0f)).setDuration(FLIP_DURATION_OUT),
				mScrollView, View.INVISIBLE));
		mSettingsButtonAnim = start(setVisibilityWhenDone(
				ObjectAnimator.ofFloat(mSettingsButton, View.ALPHA, 0f)
						.setDuration(FLIP_DURATION), mScrollView,
				View.INVISIBLE));
		mNotificationButton.setVisibility(View.VISIBLE);
		mNotificationButtonAnim = start(ObjectAnimator.ofFloat(
				mNotificationButton, View.ALPHA, 1f).setDuration(FLIP_DURATION));
		mClearButtonAnim = start(setVisibilityWhenDone(
				ObjectAnimator.ofFloat(mClearButton, View.ALPHA, 0f)
						.setDuration(FLIP_DURATION), mClearButton,
				View.INVISIBLE));
		mNotificationPanel.postDelayed(new Runnable() {
			public void run() {
				updateCarrierLabelVisibility(false);
	...
	void makeExpandedInvisible() {
		if (SPEW)
			Slog.d(TAG, "makeExpandedInvisible: mExpandedVisible="
	...
	public void showCling() {
		...
		final ViewStub stub = (ViewStub) mStatusBarWindow
				.findViewById(R.id.status_bar_cling_stub);
		if (stub == null) {
			mClingShown = true;
		...
		mHandler.postDelayed(new Runnable() {
			...
			public void run() {
				...
				mCling.findViewById(R.id.ok).setOnClickListener(
	...
	private void notifyUiVisibilityChanged() {
		try {
			mWindowManagerService
					.statusBarVisibilityChanged(mSystemUiVisibility);
		} catch (RemoteException ex) {
		}
	}
	...
	private class MyTicker extends Ticker {
		...
		public void tickerStarting() {
			...
			mStatusBarContents.startAnimation(loadAnim(
					com.android.internal.R.anim.push_up_out, null));
			mCenterClock.startAnimation(loadAnim(
					com.android.internal.R.anim.push_up_out, null));
		}
		...
		public void tickerHalting() {
			...
			mTickerView.setVisibility(View.GONE);
			mStatusBarContents.startAnimation(loadAnim(
					com.android.internal.R.anim.fade_in, null));
	...
	public void updateExpandedViewPos(int thingy) {
		...
		mNotificationPanel
				.setMinimumHeight((int) (mNotificationPanelMinHeightFrac * mCurrentDisplaySize.y));
	...
	void updateDisplaySize() {
		...
		if (DEBUG_GESTURES) {
			mGestureRec.tag("display", String.format("%dx%d",
	...
	private View.OnClickListener mClockClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			try {
				...
				// Should handle the AOSP clock as well here
				try {
					launchActivitySafely(new Intent(Intent.ACTION_MAIN)
							.setComponent(new ComponentName(
	...
	private View.OnClickListener mDateViewClickListener = new View.OnClickListener() {
		...
		public void onClick(View v) {
			try {
				...
				// Should handle the AOSP calendar as well here
				try {
					launchActivitySafely(new Intent(Intent.ACTION_MAIN)
							.setComponent(new ComponentName(
		...
		public void onClick(View v) {
			if (mHasSettingsPanel) {
				...
				startActivityDismissingKeyguard(new Intent(
						android.provider.Settings.ACTION_SETTINGS), true);
			}
		}
	};

	// private View.OnClickListener mClockClickListener = new
	...
	private void launchActivitySafely(Intent intent)
	...
	private void resetUserSetupObserver() {
		...
		mContext.getContentResolver().registerContentObserver(
				...
				true, mUserSetupObserver, mCurrentUserId);
	}
	...
	protected void loadDimens() {
		...
		mNotificationPanelMarginPx = (int) res
		...
		mSettingsPanelGravity = res
				.getInteger(R.integer.settings_panel_layout_gravity);
		...
		if (mNotificationPanelMinHeightFrac < 0f
				|| mNotificationPanelMinHeightFrac > 1f) {
	...
	protected boolean shouldDisableNavbarGestures() {
		return !isDeviceProvisioned() || mExpandedVisible
