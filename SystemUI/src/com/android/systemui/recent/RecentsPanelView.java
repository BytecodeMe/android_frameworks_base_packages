...
public class RecentsPanelView extends RelativeLayout implements OnItemClickListener, RecentsCallback,
    ...
    private void addUninstallMenuOption(final Menu menu, final ApplicationInfo info, final PackageManager pm) {
        final DevicePolicyManager mDpm = (DevicePolicyManager)getContext().getSystemService(Context.DEVICE_POLICY_SERVICE);
        PackageInfo mPackageInfo = null;
        try {
            mPackageInfo = pm.getPackageInfo(info.packageName,
                    PackageManager.GET_DISABLED_COMPONENTS |
                    PackageManager.GET_UNINSTALLED_PACKAGES |
                    PackageManager.GET_SIGNATURES);
        } catch (NameNotFoundException e) {
            Log.e(TAG, "Exception when retrieving package:" + info.packageName, e);
            return; // doesn't matter why this happened, just run away
        }
        boolean mUpdatedSysApp = (info.flags & ApplicationInfo.FLAG_UPDATED_SYSTEM_APP) != 0;
        boolean mEnabled = false;
        MenuItem mi = null;
        if (mUpdatedSysApp) {
            mi = menu.add(R.string.app_factory_reset);
            mEnabled = true;
        } else {
            if ((info.flags & ApplicationInfo.FLAG_SYSTEM) != 0) {
                try {
                    ...
                    if ((homes != null && homes.size() > 0) ||
                        ...
                        // make sure this shows disabled for core apps
                        mi = menu.add(R.string.disable);
                    } else if (info.enabled) {
                        mi = menu.add(R.string.disable);
                        mEnabled = true;
                    } else {
                        mi = menu.add(R.string.enable);
                        mEnabled = true;
                    }
                ...
                mi = menu.add(R.string.uninstall);
                mEnabled = true;
            }
        }
    ...
    public void handleLongPress(
        ...
        if (viewHolder != null) {
            ...
            //only allow a user to uninstall non-system apps from here, but 
            //allow them to disable system ones if they are non-core
            addUninstallMenuOption(popup.getMenu(), ad.resolveInfo.activityInfo.applicationInfo, pm);
            ...
            popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                public boolean onMenuItemClick(MenuItem item) {
                    if (item.getItemId() == R.id.recent_remove_item) {
                        ...
                        mBar.animateCollapse();
                    } else if (item.getTitle().equals(getResources().getString(R.string.disable))) {
                        mRecentsContainer.removeViewInLayout(selectedView);
                        mBar.animateCollapse();
                        pm.setApplicationEnabledSetting(ad.packageName, 
                                PackageManager.COMPONENT_ENABLED_STATE_DISABLED_USER, 0);
                    } else if (item.getTitle().equals(getResources().getString(R.string.uninstall)) ||
                            item.getTitle().equals(getResources().getString(R.string.app_factory_reset))) {
                        mRecentsContainer.removeViewInLayout(selectedView);
                        mBar.animateCollapse();
                        uninstallApplication(ad.packageName);
                    } else {
                        return false;
                    }
                    return true;
